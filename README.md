...
```bash
curl --request POST 'https://********.execute-api.us-east-1.amazonaws.com/dev/users' --header 'Content-Type: application/json' --data-raw '{"name": "John", "userId": someUserId"}'
```
...
```bash
curl --location --request GET 'https://*********.execute-api.us-east-1.amazonaws.com/dev/users/8164'
```
